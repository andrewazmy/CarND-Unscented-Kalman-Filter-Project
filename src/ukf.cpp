...
UKF::UKF() {
  ...
  */
  ...
  // Sigma points weight vector initialization
  weights_ = VectorXd(2 * n_aug_ + 1);

  // Initialize predicted sigma points matrix with correct dimensions
...
void UKF::Prediction(double delta_t) {
  ...
  */
  ...
  lambda_ = 3 - n_x_;
  //set first column of sigma point matrix to the state mean
  Xsig.col(0) = x_;
  ...
  // EXTRACT MEAN AND COVARIANCE FROM PREDICTED SIGMA POINTS

  //set weights
  for (int i=0; i<2*n_aug_+1; i++) {  //2n+1 weights
    double weight = 0.0;
    if (i == 0){
        weight = lambda_/(lambda_+n_aug_);
    }
    else{
